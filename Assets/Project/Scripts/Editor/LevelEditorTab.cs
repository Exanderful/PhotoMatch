...
namespace GEM.Editor
{
    ...
    public class LevelEditorTab : EditorTab
    {
        ...
        {
            var editorImagesPath = new DirectoryInfo(Application.dataPath + "/Project/Editor/Resources");
        ...
        private void DrawMenu()
        {
            ...
            if (GUILayout.Button("Новый", GUILayout.Width(100), GUILayout.Height(50)))
            {
            ...
            {
                var path = EditorUtility.OpenFilePanel("Открыть уровень", Application.dataPath + "/Project/Resources/Levels",
                    "json");
            ...
            {
                SaveLevel(Application.dataPath + "/Project/Resources");
            }
        ...
        {
            GUILayout.BeginVertical();
            EditorGUILayout.LabelField("Основа", EditorStyles.boldLabel);
            GUILayout.BeginHorizontal(GUILayout.Width(300));
            EditorGUILayout.HelpBox(
                "Основные настройки этого уровня.",
                MessageType.Info);
            GUILayout.EndHorizontal();

            GUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(new GUIContent("Номер", "Номер текущего уровня."),
            ...
            GUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(new GUIContent("Тип", "Тип текущего уровня."),
                GUILayout.Width(EditorGUIUtility.labelWidth));
            ...
            {
                EditorGUILayout.LabelField(new GUIContent("Ходы", "Максимальное количество ходов текущего уровня."),
                    GUILayout.Width(EditorGUIUtility.labelWidth));
            }
            else if (currentLevel.limitType == LimitType.Time)
            {
                EditorGUILayout.LabelField(new GUIContent("Время", "Максимальное количество секунд на текущий уровень."),
                    GUILayout.Width(EditorGUIUtility.labelWidth));
            }
            ...
            GUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(new GUIContent("Счет для 1 звезды", "Счет нужный для достижения 1 звезды."),
                //GUILayout.Width(EditorGUIUtility.labelWidth));
                GUILayout.Width(120));
            ...
            GUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(new GUIContent("Счет для 2 звезд", "Счет нужный для достижения 2 звезд."),
                //GUILayout.Width(EditorGUIUtility.labelWidth));
                GUILayout.Width(120));
            ...
            GUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(new GUIContent("Счет для 3 звезд", "Счет нужный для достижения 3 звезд."),
                //GUILayout.Width(EditorGUIUtility.labelWidth));
                GUILayout.Width(120));
        ...
        private void DrawInGameBoosterSettings()
        {
            ...
            EditorGUILayout.LabelField("Внутриигровые бустеры", EditorStyles.boldLabel);
            GUILayout.BeginHorizontal(GUILayout.Width(300));
            EditorGUILayout.HelpBox(
                "Настройки бустеров текущего уровня.",
                MessageType.Info);
            GUILayout.EndHorizontal();
        ...
        private void DrawGoalSettings()
        {
            ...
            GUILayout.BeginHorizontal(GUILayout.Width(300));
            EditorGUILayout.HelpBox(
                "Этот список определяет цели которые нужны игроку для завершения уровня.",
                MessageType.Info);
            GUILayout.EndHorizontal();
            ...
            if (currentLevel.limitType == LimitType.Moves)
            {
                ...
                GUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(new GUIContent("Дополнительные награды",
                        "Включите это если хотите чтобы были долнительные награды за оставшиеся ходы по окончанию уровня."),
                    //GUILayout.Width(EditorGUIUtility.labelWidth));
                    GUILayout.Width(170));
                ...
                {
                    GUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField(new GUIContent("Тип", "Тип дополнительной награды."),
                        GUILayout.Width(EditorGUIUtility.labelWidth));
        ...
        private void DrawAvailableColorBlockSettings()
        {
            ...
            EditorGUILayout.LabelField("Доступные цвета", EditorStyles.boldLabel);
            GUILayout.BeginHorizontal(GUILayout.Width(300));
            EditorGUILayout.HelpBox(
                "Этот список определяет доступны цвета когда появляется новая рандомная фотка.",
                MessageType.Info);
            GUILayout.EndHorizontal();
            ...
            GUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(new GUIContent("Шанс коллекционки",
                    "Рандомный шанс появления коллекционки."),
                //GUILayout.Width(EditorGUIUtility.labelWidth));
                GUILayout.Width(140));
        ...
        private void DrawLevelEditor()
        {
            ...
            GUILayout.BeginHorizontal(GUILayout.Width(300));
            EditorGUILayout.HelpBox(
                "Настройки лейаута текущего уровня.",
                MessageType.Info);
            GUILayout.EndHorizontal();

            GUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(new GUIContent("Ширина", "Ширина текущего уровня."),
                GUILayout.Width(EditorGUIUtility.labelWidth));
            ...
            GUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(new GUIContent("Высота", "Высота текущего уровня."),
                GUILayout.Width(EditorGUIUtility.labelWidth));
            ...
            {
                GUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(new GUIContent("Фотка", "Текущий тип фотки."),
                    GUILayout.Width(EditorGUIUtility.labelWidth));
            ...
            {
                GUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(new GUIContent("Спешл фотка", "Текущий тип специальной фотки."),
                    GUILayout.Width(EditorGUIUtility.labelWidth));
            ...
            {
                GUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(new GUIContent("Спешл блок", "Текущий тип специального блока."),
                    GUILayout.Width(EditorGUIUtility.labelWidth));
            ...
            {
                GUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(new GUIContent("Коллекционка", "Текущий тип коллекционки."),
                    GUILayout.Width(EditorGUIUtility.labelWidth));
        ...
        {
            goalList = SetupReorderableList("Цели", currentLevel.goals, ref currentGoal, (rect, x) =>
            {
        ...
        {
            availableCandyColorsList = SetupReorderableList("Доступные цвета", currentLevel.availableColors, ref currentCandyColor, (rect, x) =>
            {
                EditorGUI.LabelField(new Rect(rect.x, rect.y, 200, EditorGUIUtility.singleLineHeight),
                    x.ToString());
            },
            ...
            availableCandyColorsList.onRemoveCallback = l =>
            {
                ...
                {
                    EditorUtility.DisplayDialog("Ахтунг", "Вам нужен хотя бы один блок одного цвета.", "Ok");
                }
                else
                {
                    if (!EditorUtility.DisplayDialog("Ахтунг!",
                        "Вы уверены что хотите удалить это?", "Да", "Нет"))
                    {
                        return;
                    }
                    currentCandyColor = CandyColor.Blue;
        ...
        private void CreateColorBlockTypeCallback(object obj)
        {
            ...
            {
                EditorUtility.DisplayDialog("Ахтунг", "Этот цвет уже есть в списке.", "Ok");
            }
            else
            {
