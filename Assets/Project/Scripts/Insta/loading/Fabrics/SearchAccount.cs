...
public class SearchAccount : Iloading
{
    ...
    public IEnumerator Loading(string key)
    {
        ...
        {
            Debug.Log(key);
            UnityWebRequest request = UnityWebRequest.Get("https://www.instagram.com/graphql/query/?query_id=17888483320059182&id=" + _accId.graphql.user.id + "&first=6");
            yield return request.SendWebRequest();

            var dyn = JsonConvert.DeserializeObject<Assets.Accounts.LoadImages.RootObject>(request.downloadHandler.text);

            if (dyn.data.user.edge_owner_to_timeline_media.edges.Count >= 6)
            {
...
//                History.SaveToHistory(posts.AccountKey, 0, 0);
            ...
            {
                posts.AccountKey = DownloadManager.less6Error;
            }
    ...
    {
        if (posts.AccountKey.Contains(DownloadManager.less6Error) || posts.AccountKey.Contains(DownloadManager.notFoundError))
            return true;
        else
            return false;
    }
